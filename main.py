...
def word_search(text: str, word: str,num_of_Err:int=0, Ignore_case:bool=True, replacement:str ='')->list:#text- текст в котором ищем; word- слово которое ищем; num_of_Err- количество допустимых ошибок; Ignore_case- игнорироватьгнорировать ли регистр (если True то игнорировать); replacement- если его указать, то все найденные слова будут заменены на replacement
    ...
    offers=split(r'[.?!]+', text) #Делит текст на преложения. re надо для того, чтобы не писать 5 split
            ...
            if (len(u) - len_of_word)**2<=0: #Данное условие сравнивает длину искомого слова с длиной конкретного слова, это надо будет для ввода с ошибкой (если пользователь забыл пару букв)
                ...
                if counter-abs(difference)>=len_of_word- num_of_Err:
                    ...
                    for j in range(1,len(u)+1):
                        # print('u')
                        for j2 in range(1,num_of_Err+1):
                            u3=u[0:j]+' '*j2+u[j:len(u)] 
                            # print(u3)
                            # print(j2)
                            ml=min(len_of_word, len(u3))
                            for o in range(ml): 
                                if word[o] == u3[o]:
                                    counter+=1
                                    # print(u3[o])
                            if counter>=len(u3)-num_of_Err:
                                break
                            else:
                                counter=0
                        if counter>=len(u3)- num_of_Err:
...
tex='''На Пьера опять нашла та тоска, которой он так боялся. Он три дня после произнесения своей речи в ложе лежал дома на диване, никого не принимая и никуда не выезжая.
...
# tex='I have a sister, her name is Marinad. Das is good! Ладно. Расцветали, яблони и груши. Попплыли туманны над рекой?'
print(word_search(tex,'Пьера', 2,True,'' ))
